...
This is the first release of Mdstar to pypi. Since the last release, we have greatly improved 
...
* **Reduce the use of asyncio to a minimum**, improving stability and reducing the need for workarounds
   ...
   * Switch from fastapi (which uses asyncio) to Flask in the Mdstar dashboard
...
* Configuration:
   ...
   * Mdstar config, modes and contexts can now be adjusted from the user's home directory.
...
* **Overhaul and major improvement of editing tools!**
  This represents a very important change in Mdstar. Symbols can now be addressed by their `name_path` (including nested ones)
...
* Dashboard as web app and possibility to shut down Mdstar from it (or the old log GUI).
* Possibility to index your project beforehand, accelerating Mdstar's tools.
...
* Use **process isolation** to fix stability issues and deadlocks (see #170). 
  This uses separate process for the MCP server, the Mdstar agent and the dashboard in order to fix asyncio-related issues.

# 2025-05-24

* Important new feature: **configurability of mode and context**, allowing better integration in a variety of clients.
  See corresponding section in readme - Mdstar can now be integrated in IDE assistants in a more productive way. 
...
* Mdstar core:
    ...
    * New tool `RestartLanguageServerTool` for restarting the language server (in case of other sources of editing apart from Mdstar)
...
* Mdstar core:
    ...
    * Native support for gitignore files for configuring files to be ignored by mdstar. See also
      in *Language Servers* section below.
    * **Major Feature**: Allow Mdstar to switch between projects (project activation)
        * Add central Mdstar configuration in `mdstar_config.yml`, which 
...
* Agno: 
    ...
    * Fix Mdstar GUI log window not capturing logs after initialization
