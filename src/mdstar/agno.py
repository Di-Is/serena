...
from mdstar.agent import MdstarAgent, Tool
from mdstar.config.context_mode import MdstarAgentContext
...
class MdstarAgnoToolkit(Toolkit):
    def __init__(self, mdstar_agent: MdstarAgent):
        super().__init__("Mdstar")
        for tool in mdstar_agent.get_exposed_tool_instances():
...
class MdstarAgnoAgentProvider:
    ...
    def get_agent(cls, model: Model) -> Agent:
        """
        Returns the singleton instance of the Mdstar agent or creates it with the given parameters if it doesn't exist.
        ...
        with cls._lock:
            ...
            # change to Mdstar root
            os.chdir(REPO_ROOT)

            load_dotenv()

            parser = argparse.ArgumentParser(description="Mdstar coding assistant")
            ...
            with LogTime("Loading Mdstar agent"):
                try:
                    mdstar_agent = MdstarAgent(project_file, context=MdstarAgentContext.load("agent"))
            ...
            agno_agent = Agent(
                ...
                storage=SqliteStorage(table_name="mdstar_agent_sessions", db_file=str(sql_db_path)),
                description="A fully-featured coding assistant",
                tools=[MdstarAgnoToolkit(mdstar_agent)],
                # The tool calls will be shown in the UI anyway since whether to show them is configurable per tool
                # To see detailed logs, you should use the mdstar logger (configure it in the project file path)
                show_tool_calls=False,
                markdown=True,
                system_message=mdstar_agent.create_system_prompt(),
                telemetry=False,
                memory=AgentMemory(),
