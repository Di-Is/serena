...
from mdstar.constants import (
    ...
    MDSTAR_MANAGED_DIR_NAME,
    PROJECT_TEMPLATE_FILE,
    REPO_ROOT,
    SELENA_CONFIG_TEMPLATE_FILE,
)
from mdstar.util.general import load_yaml, save_yaml
from mdstar.util.inspection import determine_programming_language_composition
...
class MdstarPaths:
    """
    Provides paths to various Mdstar-related directories and files.
    """

    def __init__(self) -> None:
        self.user_config_dir: str = MDSTAR_MANAGED_DIR_IN_HOME
        """
        the path to the user's Mdstar configuration directory, which is typically ~/.mdstar
        """
...
class ToolSet:
    ...
    def default(cls) -> "ToolSet":
        ...
        """
        from mdstar.tools import ToolRegistry
...
class MdstarConfigError(Exception):
    pass


def get_mdstar_managed_in_project_dir(project_root: str | Path) -> str:
    return os.path.join(project_root, MDSTAR_MANAGED_DIR_NAME)
...
class ProjectConfig(ToolInclusionDefinition, ToStringMixin):
    ...
    MDSTAR_DEFAULT_PROJECT_FILE = "project.yml"
            ...
            if project_language is None:
                ...
                if len(language_composition) == 0:
                    raise ValueError(
                        ...
                        f"To use Mdstar with this project, you need to either:\n"
                        f"1. Add source files in Markdown format (.md, .mdx, .markdown)\n"
    ...
    def rel_path_to_project_yml(cls) -> str:
        return os.path.join(MDSTAR_MANAGED_DIR_NAME, cls.MDSTAR_DEFAULT_PROJECT_FILE)
...
class RegisteredProject(ToStringMixin):
    def __init__(self, project_root: str, project_config: "ProjectConfig", project_instance: Optional["Project"] = None) -> None:
        """
        Represents a registered project in the Mdstar configuration.
...
@dataclass(kw_only=True)
class MdstarConfig(ToolInclusionDefinition, ToStringMixin):
    """
    Holds the Mdstar agent configuration, which is typically loaded from a YAML configuration file
    ...
    CONFIG_FILE = "mdstar_config.yml"
    CONFIG_FILE_DOCKER = "mdstar_config.docker.yml"  # Docker-specific config file; auto-generated if missing, mounted via docker-compose for user customization
    ...
    def generate_config_file(cls, config_file_path: str) -> None:
        """
        Generates a Mdstar configuration file at the specified path from the template file.

        :param config_file_path: the path where the configuration file should be generated
        """
        log.info(f"Auto-generating Mdstar configuration file in {config_file_path}")
    ...
    def _determine_config_file_path(cls) -> str:
        """
        :return: the location where the Mdstar configuration file is stored/should be stored
        """
        if is_running_in_docker():
            return os.path.join(REPO_ROOT, cls.CONFIG_FILE_DOCKER)
        else:
            config_path = os.path.join(MDSTAR_MANAGED_DIR_IN_HOME, cls.CONFIG_FILE)
            ...
            if not os.path.exists(config_path):
                ...
                if os.path.exists(old_config_path):
                    log.info(f"Moving Mdstar configuration file from {old_config_path} to {config_path}")
    ...
    @classmethod
    def from_config_file(cls, generate_if_missing: bool = True) -> "MdstarConfig":
        """
        Static constructor to create MdstarConfig from the configuration file
        """
        ...
        if not os.path.exists(config_file_path):
            if not generate_if_missing:
                raise FileNotFoundError(f"Mdstar configuration file not found: {config_file_path}")
            log.info(f"Mdstar configuration file not found at {config_file_path}, autogenerating...")
            cls.generate_config_file(config_file_path)

        # load the configuration
        log.info(f"Loading Mdstar configuration from {config_file_path}")
        try:
            loaded_commented_yaml = load_yaml(config_file_path, preserve_comments=True)
        except Exception as e:
            raise ValueError(f"Error loading Mdstar configuration from {config_file_path}: {e}") from e
        ...
        if "projects" not in loaded_commented_yaml:
            raise MdstarConfigError("`projects` key not found in Mdstar configuration. Please update your `mdstar_config.yml` file.")
    ...
    def add_project_from_path(self, project_root: Path | str) -> "Project":
        """
        Add a project to the Mdstar configuration from a given path. Will raise a FileExistsError if a
    ...
    def remove_project(self, project_name: str) -> None:
        ...
        for i, project in enumerate(list(self.projects)):
            ...
            raise ValueError(f"Project '{project_name}' not found in Mdstar configuration; valid project names: {self.project_names}")
        self.save()
