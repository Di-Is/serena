...
from mdstar.config.mdstar_config import ToolInclusionDefinition
...
class MdstarAgentMode(ToolInclusionDefinition, ToStringMixin):
    ...
    It is formatted by the agent (see MdstarAgent._format_prompt()).
    """
    description: str = ""
    ...
    def get_path(cls, name: str) -> str:
        ...
        own_yaml_path = os.path.join(MDSTARS_OWN_MODE_YAMLS_DIR, fname)
        if not os.path.exists(own_yaml_path):
            raise FileNotFoundError(
                f"Mode {name} not found in {USER_MODE_YAMLS_DIR} or in {MDSTARS_OWN_MODE_YAMLS_DIR}."
    ...
    def list_registered_mode_names(cls, include_user_modes: bool = True) -> list[str]:
        """Names of all registered modes (from the corresponding YAML files in the mdstar repo)."""
        modes = [f.stem for f in Path(MDSTARS_OWN_MODE_YAMLS_DIR).glob("*.yml") if f.name != "mode.template.yml"]
...
@dataclass(kw_only=True)
class MdstarAgentContext(ToolInclusionDefinition, ToStringMixin):
    ...
    def get_path(cls, name: str) -> str:
        ...
        own_yaml_path = os.path.join(MDSTARS_OWN_CONTEXT_YAMLS_DIR, fname)
        if not os.path.exists(own_yaml_path):
            raise FileNotFoundError(
                f"Context {name} not found in {USER_CONTEXT_YAMLS_DIR} or in {MDSTARS_OWN_CONTEXT_YAMLS_DIR}."
    ...
    @classmethod
    def from_name(cls, name: str) -> Self:
        """Load a registered Mdstar context."""
    ...
    def list_registered_context_names(cls, include_user_contexts: bool = True) -> list[str]:
        """Names of all registered contexts (from the corresponding YAML files in the mdstar repo)."""
        contexts = [f.stem for f in Path(MDSTARS_OWN_CONTEXT_YAMLS_DIR).glob("*.yml")]
...
class RegisteredContext(Enum):
    ...
    """For Mdstar running within an assistant that already has basic tools, like Claude Code, Cline, Cursor, etc."""
    DESKTOP_APP = "desktop-app"
    """For Mdstar running within Claude Desktop or a similar app which does not have built-in tools for code editing."""
    AGENT = "agent"
    """For Mdstar running as a standalone agent, e.g. through agno."""

    def load(self) -> MdstarAgentContext:
        """Load the context."""
        return MdstarAgentContext.from_name(self.value)
...
class RegisteredMode(Enum):
    ...
    def load(self) -> MdstarAgentMode:
        """Load the mode."""
        return MdstarAgentMode.from_name(self.value)
