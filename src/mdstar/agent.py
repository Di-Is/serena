"""
The Mdstar Model Context Protocol (MCP) Server
"""
...
from mdstar import mdstar_version
from mdstar.analytics import RegisteredTokenCountEstimator, ToolUsageStats
from mdstar.config.context_mode import MdstarAgentContext, MdstarAgentMode, RegisteredContext
from mdstar.config.mdstar_config import MdstarConfig, ToolInclusionDefinition, ToolSet, get_mdstar_managed_in_project_dir
from mdstar.dashboard import MdstarDashboardAPI
from mdstar.project import Project
from mdstar.prompt_factory import MdstarPromptFactory
from mdstar.tools import ActivateProjectTool, Tool, ToolMarker, ToolRegistry
from mdstar.util.inspection import iter_subclasses
...
if TYPE_CHECKING:
    from mdstar.gui_log_viewer import GuiLogViewer
...
class MemoriesManager:
    def __init__(self, project_root: str):
        self._memory_dir = Path(get_mdstar_managed_in_project_dir(project_root)) / "memories"
...
class MdstarAgent:
    def __init__(
        ...
        mdstar_config: MdstarConfig | None = None,
        context: MdstarAgentContext | None = None,
        modes: list[MdstarAgentMode] | None = None,
        ...
        :param mdstar_config: the Mdstar configuration or None to read the configuration from the default location.
        ...
        :param memory_log_handler: a MemoryLogHandler instance from which to read log messages; if None, a new one will be created
            if necessary.
        """
        # obtain mdstar configuration using the decoupled factory function
        self.mdstar_config = mdstar_config or MdstarConfig.from_config_file()

        # adjust log level
        mdstar_log_level = self.mdstar_config.log_level
        if Logger.root.level > mdstar_log_level:
            log.info(f"Changing the root logger level to {mdstar_log_level}")
            Logger.root.setLevel(mdstar_log_level)

        def get_memory_log_handler() -> MemoryLogHandler:
            nonlocal memory_log_handler
            if memory_log_handler is None:
                memory_log_handler = MemoryLogHandler(level=mdstar_log_level)
                Logger.root.addHandler(memory_log_handler)
            return memory_log_handler

        # open GUI log window if enabled
        self._gui_log_viewer: Optional["GuiLogViewer"] = None
        if self.mdstar_config.gui_log_window_enabled:
            if platform.system() == "Darwin":
                ...
                self._gui_log_viewer = GuiLogViewer("dashboard", title="Mdstar Logs", memory_log_handler=get_memory_log_handler())
                self._gui_log_viewer.start()

        # set the agent context
        if context is None:
            context = MdstarAgentContext.load_default()
        ...
        if self.mdstar_config.record_tool_usage_stats:
            token_count_estimator = RegisteredTokenCountEstimator[self.mdstar_config.token_count_estimator]
        ...
        if self.mdstar_config.web_dashboard:
            self._dashboard_thread, port = MdstarDashboardAPI(
            ...
            log.info("Mdstar web dashboard started at %s", dashboard_url)
            if self.mdstar_config.web_dashboard_open_on_launch:
        ...
        log.info(f"Starting Mdstar server (version={mdstar_version()}, process id={os.getpid()}, parent process id={os.getppid()})")
        log.info("Configuration file: %s", self.mdstar_config.config_file_path)
        log.info("Available projects: {}".format(", ".join(self.mdstar_config.project_names)))
        ...
        # limited by the Mdstar config, the context (which is fixed for the session) and JetBrains mode
        tool_inclusion_definitions: list[ToolInclusionDefinition] = [self.mdstar_config, self._context]
        ...
        if self.mdstar_config.jetbrains:
            tool_inclusion_definitions.append(MdstarAgentMode.from_name_internal("jetbrains"))
        ...
        self._task_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="MdstarAgentExecutor")
        self._task_executor_lock = threading.Lock()
        self._task_executor_task_index = 1

        # Initialize the prompt factory
        self.prompt_factory = MdstarPromptFactory()
        ...
        if modes is None:
            modes = MdstarAgentMode.load_default_modes()
        self._modes = modes
    ...
    def get_context(self) -> MdstarAgentContext:
    ...
    def set_modes(self, modes: list[MdstarAgentMode]) -> None:
        """
    ...
    def get_active_modes(self) -> list[MdstarAgentMode]:
        """
    ...
    def _update_active_tools(self) -> None:
        ...
        The base tool set already takes the Mdstar configuration and the context into account
    ...
    def is_using_language_server(self) -> bool:
        ...
        """
        return not self.mdstar_config.jetbrains
    ...
    def load_project_from_path_or_name(self, project_root_or_name: str, autogenerate: bool) -> Project | None:
        ...
        :param autogenerate: whether to autogenerate the project for the case where first argument is a directory
            which does not yet contain a Mdstar project configuration file
        :return: the project instance if it was found/could be created, None otherwise
        """
        project_instance: Project | None = self.mdstar_config.get_project(project_root_or_name)
        if project_instance is not None:
            ...
            project_instance = self.mdstar_config.add_project_from_path(project_root_or_name)
    ...
    def activate_project_from_path_or_name(self, project_root_or_name: str) -> Project:
        ...
        If the argument is a path at which no Mdstar project previously existed, the project will be created beforehand.
        ...
        if project_instance is None:
            raise ProjectNotFoundError(
                ...
                f"Existing project names: {self.mdstar_config.project_names}"
            )
    ...
    def get_current_config_overview(self) -> str:
        ...
        result_str += f"Mdstar version: {mdstar_version()}\n"
        result_str += f"Loglevel: {self.mdstar_config.log_level}, trace_lsp_communication={self.mdstar_config.trace_lsp_communication}\n"
        if self._active_project is not None:
            result_str += f"Active project: {self._active_project.project_name}\n"
        else:
            result_str += "No active project\n"
        result_str += "Available projects:\n" + "\n".join(list(self.mdstar_config.project_names)) + "\n"
        ...
        all_available_modes = MdstarAgentMode.list_registered_mode_names()
    ...
    def reset_language_server(self) -> None:
        ...
        """
        tool_timeout = self.mdstar_config.tool_timeout
        ...
        self.language_server = self._active_project.create_language_server(
            log_level=self.mdstar_config.log_level,
            ls_timeout=ls_timeout,
            trace_lsp_communication=self.mdstar_config.trace_lsp_communication,
        )
    ...
    def __del__(self) -> None:
        ...
        if not hasattr(self, "_is_initialized"):
            return
        log.info("MdstarAgent is shutting down ...")
        if self.is_language_server_running():
