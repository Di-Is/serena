...
from mdstar.config.mdstar_config import MdstarConfig, MdstarPaths, ProjectConfig
from mdstar.constants import (
    DEFAULT_CONTEXT,
    DEFAULT_MODES,
    MDSTAR_LOG_FORMAT,
    MDSTAR_MANAGED_DIR_IN_HOME,
    MDSTARS_OWN_CONTEXT_YAMLS_DIR,
    MDSTARS_OWN_MODE_YAMLS_DIR,
    PROMPT_TEMPLATES_DIR_IN_USER_HOME,
    PROMPT_TEMPLATES_DIR_INTERNAL,
    USER_CONTEXT_YAMLS_DIR,
    USER_MODE_YAMLS_DIR,
)
from mdstar.mcp import MdstarMCPFactory, MdstarMCPFactorySingleProcess
from mdstar.project import Project
from mdstar.tools import FindReferencingSymbolsTool, FindSymbolTool, GetSymbolsOverviewTool, SearchForPatternTool, ToolRegistry
from mdstar.util.logging import MemoryLogHandler
...
class TopLevelCommands(AutoRegisteringGroup):
    """Root CLI group containing the core Mdstar commands."""

    def __init__(self) -> None:
        super().__init__(name="mdstar", help="Mdstar CLI commands. You can run `<command> --help` for more info on each command.")

    @staticmethod
    @click.command("start-mcp-server", help="Starts the Mdstar MCP server.")
    ...
    def start_mcp_server(
        ...
        # initialize logging, using INFO level initially (will later be adjusted by MdstarAgent according to the config)
        ...
        formatter = logging.Formatter(MDSTAR_LOG_FORMAT)
        memory_log_handler = MemoryLogHandler()
        Logger.root.addHandler(memory_log_handler)
        stderr_handler = logging.StreamHandler(stream=sys.stderr)
        stderr_handler.formatter = formatter
        Logger.root.addHandler(stderr_handler)
        log_path = MdstarPaths().get_next_log_file_path("mcp")
        file_handler = logging.FileHandler(log_path, mode="w")
        file_handler.formatter = formatter
        Logger.root.addHandler(file_handler)

        log.info("Initializing Mdstar MCP server")
        log.info("Storing logs in %s", log_path)
        project_file = project_file_arg or project
        factory = MdstarMCPFactorySingleProcess(context=context, project=project_file, memory_log_handler=memory_log_handler)
    ...
    def print_system_prompt(project: str, log_level: str, only_instructions: bool, context: str, modes: tuple[str, ...]) -> None:
        prefix = "You will receive access to Mdstar's symbolic tools. Below are instructions for using them, take them into account."
        postfix = "You begin by acknowledging that you understood the above instructions and are ready to receive tasks."
        from mdstar.tools.workflow_tools import InitialInstructionsTool

        lvl = logging.getLevelNamesMapping()[log_level.upper()]
        logging.configure(level=lvl)
        context_instance = MdstarAgentContext.load(context)
        mode_instances = [MdstarAgentMode.load(mode) for mode in modes]
        agent = MdstarAgent(
            project=os.path.abspath(project),
            mdstar_config=MdstarConfig(web_dashboard=False, log_level=lvl),
...
class ModeCommands(AutoRegisteringGroup):
    ...
    def __init__(self) -> None:
        super().__init__(name="mode", help="Manage Mdstar modes. You can run `mode <command> --help` for more info on each command.")

    @staticmethod
    @click.command("list", help="List available modes.")
    def list() -> None:
        mode_names = MdstarAgentMode.list_registered_mode_names()
        max_len_name = max(len(name) for name in mode_names) if mode_names else 20
        for name in mode_names:
            mode_yml_path = MdstarAgentMode.get_path(name)
            is_internal = Path(mode_yml_path).is_relative_to(MDSTARS_OWN_MODE_YAMLS_DIR)
    ...
    def create(name: str, from_internal: str) -> None:
        ...
        src = (
            os.path.join(MDSTARS_OWN_MODE_YAMLS_DIR, f"{from_internal}.yml")
            if from_internal
            else os.path.join(MDSTARS_OWN_MODE_YAMLS_DIR, "mode.template.yml")
        )
        if not os.path.exists(src):
            raise FileNotFoundError(
                f"Internal mode '{from_internal}' not found in {MDSTARS_OWN_MODE_YAMLS_DIR}. Available modes: {MdstarAgentMode.list_registered_mode_names()}"
            )
    ...
    def edit(mode_name: str) -> None:
        ...
        if not os.path.exists(path):
            if mode_name in MdstarAgentMode.list_registered_mode_names(include_user_modes=False):
                click.echo(
...
class ContextCommands(AutoRegisteringGroup):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="context", help="Manage Mdstar contexts. You can run `context <command> --help` for more info on each command."
        )

    @staticmethod
    @click.command("list", help="List available contexts.")
    def list() -> None:
        context_names = MdstarAgentContext.list_registered_context_names()
        max_len_name = max(len(name) for name in context_names) if context_names else 20
        for name in context_names:
            context_yml_path = MdstarAgentContext.get_path(name)
            is_internal = Path(context_yml_path).is_relative_to(MDSTARS_OWN_CONTEXT_YAMLS_DIR)
    ...
    def create(name: str, from_internal: str) -> None:
        ...
        src = (
            os.path.join(MDSTARS_OWN_CONTEXT_YAMLS_DIR, f"{from_internal}.yml")
            if from_internal
            else os.path.join(MDSTARS_OWN_CONTEXT_YAMLS_DIR, "context.template.yml")
        )
        if not os.path.exists(src):
            raise FileNotFoundError(
                f"Internal context '{from_internal}' not found in {MDSTARS_OWN_CONTEXT_YAMLS_DIR}. Available contexts: {MdstarAgentContext.list_registered_context_names()}"
            )
        os.makedirs(os.path.dirname(dest), exist_ok=True)
        shutil.copyfile(src, dest)
    ...
    def edit(context_name: str) -> None:
        ...
        if not os.path.exists(path):
            if context_name in MdstarAgentContext.list_registered_context_names(include_user_contexts=False):
                click.echo(
...
class MdstarConfigCommands(AutoRegisteringGroup):
    """Group for 'config' subcommands."""

    def __init__(self) -> None:
        super().__init__(name="config", help="Manage Mdstar configuration.")

    @staticmethod
    @click.command(
        "edit", help="Edit mdstar_config.yml in your default editor. Will create a config file from the template if no config is found."
    )
    def edit() -> None:
        config_path = os.path.join(MDSTAR_MANAGED_DIR_IN_HOME, "mdstar_config.yml")
        if not os.path.exists(config_path):
            MdstarConfig.generate_config_file(config_path)
...
class ProjectCommands(AutoRegisteringGroup):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="project", help="Manage Mdstar projects. You can run `project <command> --help` for more info on each command."
        )
    ...
    @staticmethod
    @click.command("index-deprecated", help="Deprecated alias for 'mdstar project index'.")
    @click.argument("project", type=click.Path(exists=True), default=os.getcwd(), required=False)
    ...
    def index_deprecated(project: str, log_level: str, timeout: float) -> None:
        click.echo("Deprecated! Use `mdstar project index` instead.")
        ProjectCommands._index_project(project, log_level, timeout=timeout)
    ...
    def _index_project(project: str, log_level: str, timeout: float) -> None:
        ...
        log_file = os.path.join(project, ".mdstar", "logs", "indexing.txt")
    ...
    def health_check(project: str) -> None:
        ...
        log_dir = os.path.join(project_path, ".mdstar", "logs", "health-checks")
        ...
        with FileLoggerContext(log_file, append=False, enabled=True):
            ...
            try:
                # Create MdstarAgent with dashboard disabled
                log.info("Creating MdstarAgent with disabled dashboard...")
                config = MdstarConfig(gui_log_window_enabled=False, web_dashboard=False)
                agent = MdstarAgent(project=project_path, mdstar_config=config)
                log.info("MdstarAgent created successfully")
...
class ToolCommands(AutoRegisteringGroup):
    ...
    def __init__(self) -> None:
        super().__init__(
            name="tools",
            help="Commands related to Mdstar's tools. You can run `mdstar tools <command> --help` for more info on each command.",
        )
    ...
    def description(tool_name: str, context: str | None = None) -> None:
        ...
        mdstar_context = None
        if context:
            mdstar_context = MdstarAgentContext.load(context)

        agent = MdstarAgent(
            project=None,
            mdstar_config=MdstarConfig(web_dashboard=False, log_level=logging.INFO),
            context=mdstar_context,
        )
        tool = agent.get_tool_by_name(tool_name)
        mcp_tool = MdstarMCPFactory.make_mcp_tool(tool)
        click.echo(mcp_tool.description)


class PromptCommands(AutoRegisteringGroup):
    def __init__(self) -> None:
        super().__init__(name="prompts", help="Commands related to Mdstar's prompts that are outside of contexts and modes.")
    ...
    def list() -> None:
        mdstar_prompt_yaml_names = [os.path.basename(f) for f in glob.glob(PROMPT_TEMPLATES_DIR_INTERNAL + "/*.yml")]
        for prompt_yaml_name in mdstar_prompt_yaml_names:
    ...
    @staticmethod
    @click.command("create-override", help="Create an override of an internal prompts yaml for customizing Mdstar's prompts")
    ...
    def create_override(prompt_yaml_name: str) -> None:
        ...
        mdstar_prompt_yaml_path = os.path.join(PROMPT_TEMPLATES_DIR_INTERNAL, prompt_yaml_name)
        shutil.copyfile(mdstar_prompt_yaml_path, user_prompt_yaml_path)
    ...
    def list_overrides() -> None:
        ...
        for file_path in override_files:
            if os.path.basename(file_path) in mdstar_prompt_yaml_names:
...
def get_help() -> str:
    """Retrieve the help text for the top-level Mdstar CLI."""
    return top_level.get_help(click.Context(top_level, info_name="mdstar"))
