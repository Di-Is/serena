...
from mdstar.prompt_factory import PromptFactory
from mdstar.symbol import LanguageServerSymbolRetriever
from mdstar.util.class_decorators import singleton
...
if TYPE_CHECKING:
    from mdstar.agent import LinesRead, MdstarAgent, MemoriesManager
    from mdstar.code_editor import CodeEditor
...
class Component(ABC):
    def __init__(self, agent: "MdstarAgent"):
        self.agent = agent
...
class Tool(Component):
    ...
    def apply_ex(self, log_call: bool = True, catch_exceptions: bool = True, **kwargs) -> str:  # type: ignore
                ...
                if not isinstance(self, ToolMarkerDoesNotRequireActiveProject):
                    if self.agent._active_project is None:
                        return (
                            ...
                            + f"{self.agent.mdstar_config.project_names}"
                        )
        ...
        return future.result(timeout=self.agent.mdstar_config.tool_timeout)
...
class EditedFileContext:
    ...
    def __init__(self, relative_path: str, agent: "MdstarAgent"):
...
class ToolRegistry:
    def __init__(self) -> None:
        ...
        for cls in iter_subclasses(Tool):
            if not cls.__module__.startswith("mdstar.tools"):
                continue
