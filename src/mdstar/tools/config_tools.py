...
from mdstar.config.context_mode import MdstarAgentMode
from mdstar.tools import Tool, ToolMarkerDoesNotRequireActiveProject, ToolMarkerOptional
    ...
    def apply(self, project: str) -> str:
        ...
        if active_project.is_newly_created:
            result_str = (
                ...
                f"The project's Mdstar configuration is in {active_project.path_to_project_yml()}. In particular, you may want to edit the project name and the initial prompt."
            )
        else:
...
class RemoveProjectTool(Tool, ToolMarkerDoesNotRequireActiveProject, ToolMarkerOptional):
    """
    Removes a project from the Mdstar configuration.
    """
    ...
    def apply(self, project_name: str) -> str:
        ...
        """
        self.agent.mdstar_config.remove_project(project_name)
