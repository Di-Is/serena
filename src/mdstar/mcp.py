...
from mdstar.agent import (
    MdstarAgent,
    MdstarConfig,
)
from mdstar.config.context_mode import MdstarAgentContext, MdstarAgentMode
from mdstar.constants import DEFAULT_CONTEXT, DEFAULT_MODES, MDSTAR_LOG_FORMAT
from mdstar.tools import Tool
from mdstar.util.exception import show_fatal_exception_safe
...
def configure_logging(*args, **kwargs) -> None:  # type: ignore
    ...
    if not logging.is_enabled():
        logging.basicConfig(level=logging.INFO, stream=sys.stderr, format=MDSTAR_LOG_FORMAT)
...
@dataclass
class MdstarMCPRequestContext:
    agent: MdstarAgent


class MdstarMCPFactory:
    def __init__(self, context: str = DEFAULT_CONTEXT, project: str | None = None):
        ...
        :param project: Either an absolute path to the project directory or a name of an already registered project.
            ...
            afterward.
        """
        self.context = MdstarAgentContext.load(context)
    ...
    def make_mcp_tool(tool: Tool, openai_tool_compatible: bool = True) -> MCPTool:
        """
        Create an MCP tool from a Mdstar Tool instance.

        :param tool: The Mdstar Tool instance to convert.
        :param openai_tool_compatible: whether to process the tool schema to be compatible with OpenAI tools
            (doesn't accept integer, needs number instead, etc.). This allows using Mdstar MCP within codex.
        """
        ...
        if openai_tool_compatible:
            parameters = MdstarMCPFactory._sanitize_for_openai_tools(parameters)
    ...
    @abstractmethod
    def _instantiate_agent(self, mdstar_config: MdstarConfig, modes: list[MdstarAgentMode]) -> None:
        pass
    ...
    def create_mcp_server(
        ...
        """
        Create an MCP server with process-isolated MdstarAgent to prevent asyncio contamination.

        :param host: The host to bind to
        :param port: The port to bind to
        :param modes: List of mode names or paths to mode files
        :param enable_web_dashboard: Whether to enable the web dashboard. If not specified, will take the value from the mdstar configuration.
        :param enable_gui_log_window: Whether to enable the GUI log window. It currently does not work on macOS, and setting this to True will be ignored then.
            If not specified, will take the value from the mdstar configuration.
        :param log_level: Log level. If not specified, will take the value from the mdstar configuration.
        :param trace_lsp_communication: Whether to trace the communication between Mdstar and the language servers.
            This is useful for debugging language server issues.
        :param tool_timeout: Timeout in seconds for tool execution. If not specified, will take the value from the mdstar configuration.
        """
        try:
            config = MdstarConfig.from_config_file()
            ...
            modes_instances = [MdstarAgentMode.load(mode) for mode in modes]
...
class MdstarMCPFactorySingleProcess(MdstarMCPFactory):
    """
    MCP server factory where the MdstarAgent and its language server run in the same process as the MCP server
    """
    ...
    def __init__(self, context: str = DEFAULT_CONTEXT, project: str | None = None, memory_log_handler: MemoryLogHandler | None = None):
        ...
        self.agent: MdstarAgent | None = None
        self.memory_log_handler = memory_log_handler

    def _instantiate_agent(self, mdstar_config: MdstarConfig, modes: list[MdstarAgentMode]) -> None:
        self.agent = MdstarAgent(
            project=self.project, mdstar_config=mdstar_config, context=self.context, modes=modes, memory_log_handler=self.memory_log_handler
        )
