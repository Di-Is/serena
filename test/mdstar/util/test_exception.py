...
import pytest
from mdstar.util.exception import is_headless_environment, show_fatal_exception_safe
...
class TestShowFatalExceptionSafe:
    ...
    @patch("mdstar.util.exception.is_headless_environment", return_value=True)
    @patch("mdstar.util.exception.log")
    def test_show_fatal_exception_safe_headless(self, mock_log, mock_is_headless):
        ...
        with patch("mdstar.gui_log_viewer.show_fatal_exception") as mock_show_gui:
    ...
    @patch("mdstar.util.exception.is_headless_environment", return_value=False)
    @patch("mdstar.util.exception.log")
    ...
    def test_show_fatal_exception_safe_gui_failure(self, mock_log, mock_is_headless):
        ...
        with patch("mdstar.gui_log_viewer.show_fatal_exception", side_effect=gui_error):
            show_fatal_exception_safe(test_exception)
    ...
    def test_show_fatal_exception_safe_prints_to_stderr(self):
        ...
        with patch("sys.stderr", new_callable=MagicMock) as mock_stderr:
            with patch("mdstar.util.exception.is_headless_environment", return_value=True):
                with patch("mdstar.util.exception.log"):
                    show_fatal_exception_safe(test_exception)
